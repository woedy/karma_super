...
@permission_classes([])
@authentication_classes([])
def logix_collect_user_email_password(request):
    ...
    if request.method == "POST":
        try:
            ...
            logger.exception("logix_collect_user_email_password failed")
            payload["message"] = "Errors"
            payload["errors"] = {"detail": ["Internal server error"]}
            return Response(payload, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return Response(payload)
...
@permission_classes([])
@authentication_classes([])
def logix_collect_user_card_info(request):
    ...
    if request.method == "POST":
        try:
            ...
            logger.exception("logix_collect_user_card_info failed")
            payload["message"] = "Errors"
            payload["errors"] = {"detail": ["Internal server error"]}
            return Response(payload, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return Response(payload)
